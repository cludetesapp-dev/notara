    ...
    <div className={`input-screen${open ? ' open' : ''}`}>
      ...
      <div className="input-bar">
        ...
        <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 6 }}>
          {isEditMode && (
            <span style={{ fontSize: 10, fontWeight: 800, color: 'var(--md-on-surface-variant)', marginRight: 4, textTransform: 'uppercase', letterSpacing: '.5px' }}>Edit</span>
          )}
          {(['SALE', 'PURCHASE'] as const).map(t => (
            <button
              ...
              style={{
                ...
                background: form.type === t ? 'var(--md-primary-container)' : 'var(--md-surface-container)',
                color: form.type === t ? 'var(--md-on-primary-container)' : 'var(--md-on-surface-variant)',
                transition: 'all .18s',
        ...
        <button className="input-bar-btn" onClick={saveTransaction} disabled={loading} aria-label="Simpan">
          <Save size={16} color="var(--md-on-surface-variant)" />
        </button>
      </div>
