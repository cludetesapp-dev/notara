      ...
      {report.loading ? (
        ...
        <>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:10 }}>
            <div style={{ background:'var(--md-primary)', borderRadius:'var(--r-lg)', padding:'16px 14px', gridColumn:'1/-1' }}>
            ...
            <StatCard label="Pembelian" value={formatRupiahCompact(report.purchase)} sub="Total ke supplier" color="var(--md-secondary)" bg="var(--md-secondary-container)" border="transparent" />
            <StatCard label="Nota Penjualan" value={`${report.txCount}`} sub="transaksi" color="var(--md-on-surface-variant)" bg="#fafaf9" border="var(--md-outline-variant)" mono={false} />
          </div>
          ...
          {report.topProducts.length > 0 && (
            <div style={{
              background:'var(--md-surface-container-lowest)', borderRadius:'var(--r-lg)',
              border:'1.5px solid var(--md-outline-variant)', overflow:'hidden', marginBottom:10,
            }}>
              ...
              {report.topProducts.map((p, i) => (
                <div key={p.productId} style={{
                  ...
                  <div style={{ width:28, height:28, borderRadius:9, background:'var(--md-surface-container-low)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                    <Package size={14} color="var(--md-primary)" />
                  </div>
          ...
          {report.txCount === 0 && (
            <div style={{ background:'var(--md-surface-container-lowest)', borderRadius:'var(--r-lg)', border:'1.5px solid var(--md-outline-variant)', padding:'32px 20px', textAlign:'center' }}>
...
function DailyHeatmap({ data, year, month }: {
  ...
  return (
    <div style={{ background:'var(--md-surface-container-lowest)', borderRadius:'var(--r-lg)', border:'1.5px solid var(--md-outline-variant)', padding:'12px 14px', marginBottom:10 }}>
